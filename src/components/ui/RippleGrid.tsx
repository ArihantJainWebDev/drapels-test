...
void main() {
    ...
    const program = new Program(gl, { vertex: vert, fragment: frag, uniforms: uniforms as any });
    const mesh = new Mesh(gl, { geometry, program });

    const resize = () => {
      const { clientWidth: w, clientHeight: h } = containerRef.current!;
      renderer.setSize(w, h);
      (uniforms.iResolution as any).value = [w, h];
    };
    ...
    const handleTouchEnd = () => { if (!mouseInteraction) return; mouseInfluenceRef.current = 0.0; };
    ...
    const render = (t: number) => {
      (uniforms.iTime as any).value = t * 0.001;
      ...
      const currentInfluence = (uniforms.mouseInfluence as any).value as number;
      const targetInfluence = mouseInfluenceRef.current;
      (uniforms.mouseInfluence as any).value = currentInfluence + (targetInfluence - currentInfluence) * 0.05;

      (uniforms.mousePosition as any).value = [mousePositionRef.current.x, mousePositionRef.current.y];
